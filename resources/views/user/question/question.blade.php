...
<body>
  ...
  <!-- ***** Header Area Start ***** -->
    ...
    <div class="container">
        ...
        <p>{{ $topic['questions'][$questionIndex] }}</p><br>

        <form action="{{ route('submit.answer', $topic['title']) }}" method="POST">
            @csrf
            <input type="hidden" name="questionIndex" value="{{ $questionIndex }}">
            <button type="submit" name="answer" value="yes">Ya</button>
            <button type="submit" name="answer" value="no">Tidak</button>
        </form>

        <!-- Menampilkan posisi pertanyaan saat ini -->
        <br><p>Pertanyaan {{ $questionIndex + 1 }} dari {{ $totalQuestions }}</p>
    </div>
